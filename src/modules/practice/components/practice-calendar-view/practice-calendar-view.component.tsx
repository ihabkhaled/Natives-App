...
export function PracticeCalendarView(props: PracticeCalendarViewProps): React.JSX.Element {
  return (
    <PageShell title={props.title} testId={TEST_IDS.practiceCalendarPage}>
      <section
        data-testid={TEST_IDS.practiceCalendarView}
        aria-label={props.title}
        className="app-practice-calendar flex flex-col gap-5"
      >
        ...
        {props.status === 'loading' ? (
          <LoadingState
            ...
            testId={TEST_IDS.practiceCalendarLoading}
            variant="list"
          />
        ) : null}
        {props.status === 'error' ? (
