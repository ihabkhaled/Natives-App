/**
 * Canonical, policy-valid documents for every `SettingKey`, mirroring the
 * backend's `test/fixtures/setting-values.fixture.ts` so both suites always
 * exercise the same shapes. The statuses document adds an active `injured`
 * entry that counts toward metrics while the weights document omits it — the
 * legitimate D3 drift that keeps the snapshot's `issues` surface exercised.
 */
...
export const VALID_ASSESSMENT_SCALE: Readonly<Record<string, unknown>> = {
  ...
  step: 1,
  bands: [
    { key: 'developing', labelEn: 'Developing', labelAr: 'قيد التطور', from: 1, to: 2 },
...
/** The permanent legacy row's stored document (predates the typed contract). */
export const LEGACY_REPORT_BRANDING: Readonly<Record<string, unknown>> = { logo: 'default' };

/** Every canonical valid document, keyed by the `SettingKey` string value. */
export const VALID_SETTING_DOCUMENTS: Readonly<Record<string, Readonly<Record<string, unknown>>>> =
  {
